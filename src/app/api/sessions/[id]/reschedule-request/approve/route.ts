...
import { approveRescheduleRequest } from "@/src/lib/scheduling/server/approveRescheduleRequest";
...
export async function POST(
  ...
  const result = await approveRescheduleRequest(supabase, parsedParams.data.id);
  if (!result.ok) {
    return NextResponse.json(
      { error: result.error },
